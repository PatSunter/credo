...
class ModelResult:
	...
	def recordFieldResult( self, fieldName, tol, errors ):
		'''Records the info required for a FieldResult in the array of stored FieldResults kept
		by the ModelResult. Returns a reference to the just-added FieldResult.'''
		fieldResult = FieldResult(fieldName, tol, errors)
		self.fieldResults.append( fieldResult )
		return fieldResult

# TODO: can the below just be collapsed into a dictionary? Then have standard write facilities.
...
class FieldResult:
	...
	def __init__( self, fieldName, tol, dofErrors ):
		...
		self.tol = float(tol)
		self.dofErrors = []
		# Allow the user to pass in just a single error value result for simple fields
	...
	def writeInfoXML( self, fieldResultsNode ):
		'''Writes information about a FieldResult into an existing, open XML doc node'''
...
def updateModelResultsXMLFieldInfo( filename, newFieldResult, prettyPrint=True ):
	...
	# only allow adding of field results
	# Because we just grabbed a reference to the root, the find will look relative to the root
	fieldResultsNode = xmlDoc.find( FieldResult.xmlListTag )
	# It may not exist, if there were no field results already, in which case grab existing
	if fieldResultsNode is None:
		fieldResultsNode = etree.SubElement( root, FieldResult.xmlListTag )
	else:
		# TODO: Check it's not in the list already
		pass
