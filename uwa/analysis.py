...
class CvgFileInfo:
	'''A simple class to store info about what fields map to what convergence files'''
...
def testConvergence( modelRun ):
	...
	# Note: ideally, we'd have some better way of knowing what these analytic soln output files
	#  would be called - perhaps the analytic plugins should create a meta report file that can
	#  be parsed and read
...
def genConvergenceFileIndex( path ):
	'''Returns a dictionary relating field names to convergence files, for all .cvg files in the given path'''
...
def checkFieldConvergence( fieldTest, cvgFileInfo ):
	...
	# Just read the last line - need to check this up, presumably this is final timestep?
	# NB: this could be done more efficiently using file.seek, if performance becomes an issue with very large
	# convergence files
