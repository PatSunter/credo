...
def getVerifyStgExePath( exeName ):
	...
	if not os.path.exists(fullExePath):
		print "Error, in %s(): executable requested %s doesn't exist/not valid in StGermain binary path (%s)." % (inspect.stack()[0][3], exeName, stgPath)
		raise IOError
...
def getStgBinPath():
	try:
		...
		print "Error in %s(): since this script needs to use StGermain executables, " \
			"please set the %s environment variable for use within UWA.\n" \
			% (inspect.stack()[0][3], STG_BINDIRKEY)
		key = keyE
		sys.exit(2)

	if not os.path.exists(stgBinPath):
		print "Error, in %s(): needed env variable %s is set, but path it's set to (%s) doesn't " \
			"exist/not valid.\n" % (inspect.stack()[0][3], STG_BINDIRKEY, stgBinPath )
		sys.exit(2)	
	elif not os.path.exists(stgBinPath+os.sep+'StGermain'):
		print "Error, in %s(): needed env variable %s is set, path it's set to (%s) is valid, " \
			"but doesn't contain the StGermain executable.\n" % (inspect.stack()[0][3], STG_BINDIRKEY, stgBinPath )
		sys.exit(2)	
