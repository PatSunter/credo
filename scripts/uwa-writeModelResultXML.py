...
def main(argv):
	...
	walltime=0
	fResults=[]
	#TODO: would be good to get these provided from a static function of uwa.modelRun ...
	...
	for arg in args[1:]:
		try:
			...
			print "UWA ModelResults XML Writer: Error with provided argument '%s', should be in" \
				" the form param=value\n" % arg
			sys.exit(2)

		if val == "":
			print "UWA ModelResults XML Writer: Error with provided argument '%s'," \
				" needs a value provided for the parameter to be written\n" % param
			sys.exit(2)

		if param in assigned:
			assigned[param]=True
			#TODO: could make this bit more maintainable by filling in dictionary directly...
			if param == 'walltime':
				walltime=val
			...
			print "UWA ModelResults XML Writer: Error with provided argument '%s'," \
				" param '%s' is not in known list of model result parameters" % (arg,param)
	...
	if not updateMode:
		for kw, val in assigned.iteritems():
			if val != True:
				print "UWA ModelResults XML Writer: Error, necessary parameter '%s' not specified.\n" % kw
				print "Parameters that need to be assigned are:"
				print assigned.keys()
				sys.exit(2)
